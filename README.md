...
OnlyCCFA 是基于 [CCFrank](https://github.com/WenyanLiu/CCFrank4dblp) 的独立 Chrome 扩展。它保留原有 CCF 等级标签能力，并把 Google 学术和 Semantic Scholar 搜索进一步变成“更适合筛论文”的工作流：在 Google 学术中深度加载多页结果，在两个搜索站点中按 CCF、SCI/JCR、中科院分区、EI、中文核心以及各方向 TOP venue 标签降噪筛选，再把结果导出为 BibTeX，或让 Zotero Connector 只识别当前筛选后的 Google 学术结果。
...
- 多源标签组合筛选。
- Google 学术 / Semantic Scholar / Crossref / arXiv BibTeX 获取、BibTeX 解析和 Zotero Connector 筛选结果兼容。
- 筛选偏好保存和未识别结果处理。
- 筛选统计信息。
- Google 学术 venue 提取。
