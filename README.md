...
OnlyCCFA 是基于 [CCFrank](https://github.com/WenyanLiu/CCFrank4dblp) 的独立 Chrome 扩展。它保留原有 CCF 等级标签能力，并把 Google 学术搜索进一步变成“更适合筛论文”的工作流：深度加载多页 Google 学术结果，按 CCF、SCI/JCR、中科院分区、EI、中文核心以及各方向 TOP venue 标签降噪筛选，再把结果导出为 BibTeX 或发送到 Zotero。
...
- 支持组合筛选 SCI、JCR Q1/Q2、中科院 1区/2区/TOP、EI、中文核心、西南交大 / SCAI / 交通运输专项目录、机器人/通信/电气/控制/机械方向 TOP，并可选择“任一匹配”或“同时满足”。
- 支持单篇复制 BibTeX、批量导出勾选论文、导出当前可见论文、导出深筛结果池，并提供实验性 Zotero 本地导入入口。
...
| BibTeX / Zotero：从筛选结果进入文献库                                                                                     | 继续下一批：可调数量、可续扫、可清空                                                                            |
| ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
...
当前内置数据是种子数据集，覆盖一批常见国际 venue、中文核心期刊、方向 TOP venue，以及西南交通大学学术期刊分级目录、计算机与人工智能学院高水平期刊 C 类目录、交通运输工程专项期刊目录的派生标签。例如 CoRL、RSS、ICRA、IROS、TRO、IJRR、RA-L、Automatica、IEEE TAC、IEEE TPEL、IEEE TWC、IEEE JSAC 等。
