...
- 支持单篇复制 BibTeX、批量导出勾选论文、导出当前可见论文、导出深筛结果池；BibTeX 直接取自 Google 学术原生引用，拿不到时会停止导出，避免用页面摘要拼出不准确引用。
- 提供实验性 Zotero 本地导入入口，导入数据同样基于 Google 学术 BibTeX，而不是 OnlyCCFA 自行补全的字段。
...
- 多源标签组合筛选。
- Google 学术 BibTeX 获取、BibTeX 解析和 Zotero item 转换。
- 筛选偏好保存和未识别结果处理。
- 筛选统计信息。
- Google 学术 venue 提取。
